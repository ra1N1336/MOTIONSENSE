...
void Render::Initialise()
{
	...
	Fonts::INTERX = Interfaces::Surface->FontCreate();
	Fonts::skrrtmenufont = Interfaces::Surface->FontCreate();
	Fonts::Tabs = Interfaces::Surface->FontCreate();
	Fonts::Verdana = Interfaces::Surface->FontCreate();   
	Fonts::Courier = Interfaces::Surface->FontCreate();
	//Fonts::kokonut = Interfaces::Surface->FontCreate();

	Interfaces::Surface->SetFontGlyphSet(Fonts::Menu, "Tahoma", 14, 900, 0, 0, FONTFLAG_ANTIALIAS);
	Interfaces::Surface->SetFontGlyphSet(Fonts::MenuBold, "Tahoma", 12, 900, 0, 0, FONTFLAG_ANTIALIAS | FONTFLAG_DROPSHADOW);
	Interfaces::Surface->SetFontGlyphSet(Fonts::ESP, "Dosis", 12, 800, 0, 0, FONTFLAG_ANTIALIAS | FONTFLAG_DROPSHADOW);
	Interfaces::Surface->SetFontGlyphSet(Fonts::INTERX, "Impact", 20, 500, 0, 0, FONTFLAG_ANTIALIAS | FONTFLAG_DROPSHADOW | FONTFLAG_OUTLINE);
	Interfaces::Surface->SetFontGlyphSet(Fonts::skrrtmenufont, "MenuFont", 40, 600, 0, 0, FONTFLAG_ANTIALIAS | FONTFLAG_DROPSHADOW);
	Interfaces::Surface->SetFontGlyphSet(Fonts::Verdana, "Verdana", 12, 900, 0, 0, FONTFLAG_ANTIALIAS);
	Interfaces::Surface->SetFontGlyphSet(Fonts::Tabs, "skrrtmenufont", 40, 600, 0, 0, FONTFLAG_ANTIALIAS | FONTFLAG_DROPSHADOW);
	Interfaces::Surface->SetFontGlyphSet(Fonts::Courier, "Verdana", 12, 900, 0, 0, FONTFLAG_ANTIALIAS);
	//Interfaces::Surface->SetFontGlyphSet(Fonts::kokonut, "KOKONUT", 24, 900, 0, 0, FONTFLAG_ANTIALIAS);
...
void D::DrawRectGradientVertical(int x, int y, int w, int h, Color c1, Color c2)
{
	D::DrawRect(x, y, w, h, c1);
	BYTE first = c2.r();
	BYTE second = c2.g();
	BYTE third = c2.b();
	for (int i = 0; i < h; i++)
	{
		...
		float a = fi / fh;
		DWORD ia = a * 255;
		D::DrawRect(x, y + i, w, 1, Color(first, second, third, ia));
	}
}
...
void D::DrawString(unsigned long font, int x, int y, Color color, DWORD alignment, const char* msg, ...)
{
	va_list va_alist;
	char buf[1024];
...
void D::Text(int x, int y, Color color, DWORD font, const wchar_t* text)
{
	Interfaces::Surface->DrawSetTextFont(font);
	Interfaces::Surface->DrawSetTextColor(color);
	Interfaces::Surface->DrawSetTextPos(x, y);
...
void D::Textf(int x, int y, Color color, DWORD font, const char* fmt, ...)
{
	if (!fmt) return; //if the passed string is null return
	if (strlen(fmt) < 2) return;
...
void D::DrawPolygonRainbow(int count, Vertex_t* Vertexs, float flSpeed, float &flRainbow)
{
	...
	D::DrawPolygon(count, Vertexs, colRainbow);
...
void D::DrawStringUnicode(DWORD font, int x, int y, Color color, bool bCenter, const wchar_t* msg, ...)
{
	int r = 255, g = 255, b = 255, a = 255;
	color.GetColor(r, g, b, a);
