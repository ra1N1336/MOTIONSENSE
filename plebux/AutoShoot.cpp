...
float AutoShoot::GetHitChance(Vector Point)
{
	...
	float SpreadCone = ActiveWeapon->GetAccuracyPenalty() * 256.0f / M_PI + ActiveWeapon->GetCSWpnData()->m_flInaccuracyMove * pLocal->GetVelocity().Length() / 3000.0f;
	...
	if (Config.g_iAutoShootMode == 0)
	{
		...
		if (Target)
		{
			...
			if (Config.g_iFakeLagFix == 1)
				pCmd->tick_count += pEntity->GetChockedPackets() + 1;
			else if (Config.g_iFakeLagFix == 2)
			{
				if (pEntity->GetChockedPackets() > 5)
				{
					AutoShoot::IsFiring = false;
					return;
				}
	...
	for (float XCoord = 0; XCoord <= XBounds; XCoord += XStep)
	{
		for (float YCoord = 0; YCoord <= YBounds; YCoord += YStep)
		{
			...
			if (Config.g_bAutoWall)
			{
				...
				if (Damage > Config.g_iAutoWall_MinDamage || Damage + 1.0f > AutoWall::GetHitgroupDamage(HitGroup) * (wpn_data->m_iDamage * (float)(pow(wpn_data->m_flRangeModifier, (CurrentPosition - LocalEyePos).Length() * 0.002))))
				{
			...
			else
			{
				...
				if (tr.m_pEnt)
				{
					...
					{
						Result.OptimalPoint[tr.hitgroup] += CurrentPosition * (float)wpn_data->m_iDamage;
						Result.DamageWeights[tr.hitgroup] += (float)wpn_data->m_iDamage;
			...
			for (float l = 0; l <= 1; l += 1.0f / LinStep)
			{
				...
				else
				{
					...
					if (tr.m_pEnt)
					{
						if (tr.m_pEnt == pTarget)
						{
							float Damage = wpn_data->m_iDamage;
	...
	if (!CurrentTarget && Config.g_bAutoWall)
	{
		...
		for (int i = 1; i < 65; i++)
		{
			...
			if (Utils::IsViableTarget(pEntity))
			{
				...
				if (Damage > HighestDamage && Damage > Config.g_iAutoWall_MinDamage || (Damage + 1.0f > (wpn_data->m_iDamage * (float)(pow(wpn_data->m_flRangeModifier, (EntityPos - LocalEyePos).Length() * 0.002))) && Damage > HighestDamage))
				{
					CurrentTarget = i;
