/////////////////////////////////////////
// zMenu 1.1, Customized for Mutiny.pw //
///////// Made by toast/zzzzz ///////////
/////////////////////////////////////////
...
namespace Menu
{
	...
	// These are just placeholders, replace them with your own variables
	bool bTest;
	int iTest;
	float fTest;
	...
	void InitializeMenu()
	{
		...
		CMainWindow* MainWindow = new CMainWindow(Vector2D(120, 120), Vector2D(480, 360), 14, XorStr("kryst4lmeth for Counter-Strike: Global Offensive"));
		{
			...
			CTab* RagebotTab = new CTab("Ragebot");
			{
				CSubTab* GeneralTab = new CSubTab("General");
				{
					...
					}
					{
						...
						RageBotExtendedColumn->AddChild(new CCombobox(XorStr("FakeLag Fix"), XorStr("Bring those plebs back up to speed"), std::vector<std::string>{XorStr("None"), XorStr("Normal")}, &Config.g_iFakeLagFix));
					}
				...
				CSubTab* AntiAimTab = new CSubTab("Anti-Aim Options");
				{
					...
					CSplitVerticalLayout* AngleOptions = new CSplitVerticalLayout(XorStr("Real Angles"), XorStr("Fake Angles"), 5);
					// AntiAimOptions Start
					{
						...
						AntiAimOptions->AddChild(new CCombobox(XorStr("Pitch Mod"), XorStr("Pitch compression modifier"), std::vector<std::string>{XorStr("None"), XorStr("Lisp"), XorStr("Angel")}, &Config.g_iAntiAim_PitchMod));
						AntiAimOptions->AddChild(new CCombobox(XorStr("Yaw Mod"), XorStr("Yaw compression modifier"), std::vector<std::string>{XorStr("None"), XorStr("Lisp"), XorStr("Angel")}, &Config.g_iAntiAim_YawMod));
					...
					// AngleOptions Start
					{
						AngleOptions->AddChild(new CCombobox(XorStr("Anti-Aim Pitch T1"), XorStr("Vertical anti-aim"), std::vector<std::string>{XorStr("None"), XorStr("Down"), XorStr("Emotion"), XorStr("FakeDown"), XorStr("FakeAngel"), XorStr("Rotary"), XorStr("Rand")}, &Config.g_iCustomAntiAim_PitchOverrideT1));
						...
						AngleOptions->AddChild(new CCombobox(XorStr("Pitch Mod T1"), XorStr("Pitch compression modifier"), std::vector<std::string>{XorStr("None"), XorStr("Lisp"), XorStr("Angel")}, &Config.g_iCustomAntiAim_PitchModOverrideTick1));
						AngleOptions->AddChild(new CCombobox(XorStr("Yaw Mod"), XorStr("Yaw Compression modifier"), std::vector<std::string>{XorStr("None"), XorStr("Lisp"), XorStr("Angel")}, &Config.g_iCustomAntiAim_RealYawModOverride));
					...
					}
					{
						...
						AngleOptions->AddChild(new CCombobox(XorStr("Anti-Aim Pitch T2"), XorStr("Vertical anti-aim (NOTE: You can't actually fake pitches)"), std::vector<std::string>{XorStr("None"), XorStr("Down"), XorStr("Emotion"), XorStr("FakeDown"), XorStr("FakeAngel"), XorStr("Rotary"), XorStr("Rand")}, &Config.g_iCustomAntiAim_PitchOverrideT2));
						...
						AngleOptions->AddChild(new CCombobox(XorStr("Pitch Mod T2"), XorStr("Pitch compression modifier  (NOTE: You can't actually fake pitches)"), std::vector<std::string>{XorStr("None"), XorStr("Lisp"), XorStr("Angel")}, &Config.g_iCustomAntiAim_PitchModOverrideTick2));
						AngleOptions->AddChild(new CCombobox(XorStr("Yaw Mod"), XorStr("Yaw Compression modifier"), std::vector<std::string>{XorStr("None"), XorStr("Lisp"), XorStr("Angel")}, &Config.g_iCustomAntiAim_FakeYawModOverride));
			...
			CTab* RageBotTabExtended = new CTab(XorStr("Debug"));
			{
				CVerticalLayout* DebugColumn = new CVerticalLayout("Debug Menu");
				{
					DebugColumn->AddChild(new CCheckbox(XorStr("Visualize Hitscan"), XorStr("RIP FPS"), &Config.g_bVisualizeHitscan));
				}
				RageBotTabExtended->AddChild(DebugColumn);
				CSplitVerticalLayout* ConfigColumn = new CSplitVerticalLayout(XorStr("Config"), XorStr("Other"), 3);
				{
					ConfigColumn->AddChild(new CCombobox(XorStr("Config Slot"), XorStr("Current config number"), std::vector<std::string>{XorStr("Slot 1"), XorStr("Slot 2"), XorStr("Slot 3"), XorStr("Slot 4"), XorStr("Slot 5")}, &ConfigSlot));
					ConfigColumn->AddChild(new CButton(XorStr("Load Config"), XorStr("Load config at slot"), LoadConfig));
					ConfigColumn->AddChild(new CButton(XorStr("Save Config"), XorStr("Save config at slot"), SaveConfig));
				}
				{

				}
				RageBotTabExtended->AddChild(ConfigColumn);
			}
			MainWindow->AddChild(RageBotTabExtended);
			CTab* MiscTab = new CTab(XorStr("Misc"));
			{
				CVerticalLayout* MiscColumn = new CVerticalLayout("Misc");
				{
					...
					MiscColumn->AddChild(new CCheckbox(XorStr("Air Stuck (UNTRUSTED)"), XorStr("Float in the air (V Key)"), &Config.g_bAirStuck));
					MiscColumn->AddChild(new CCombobox(XorStr("Clan Tag"), XorStr("Am I cool yet?"), std::vector<std::string>{XorStr("kryst4lmeth"), XorStr("skeet"), XorStr("Zeus"), XorStr("TristanHook"), XorStr("pHvH"), XorStr("NewLine"), XorStr("Valve"), XorStr("None")}, &Config.g_iTagOption));
		...
		/*
		CWindow* TestWindow = new CWindow(Vector2D(180, 180), Vector2D(360, 240), 14, XorStr("TEST"));
		{
			CTab* Tab1 = new CTab("Tab 1");
			{
				CSubTab* Tab3 = new CSubTab("Tab 3");
				{
					CVerticalLayout* Column1 = new CVerticalLayout("Column 1");
					{
						Column1->AddChild(new CCheckbox("Test", "Test Tooltip", &bTest));
						Column1->AddChild(new CCombobox("Test", "Test Tooltip", std::vector<std::string>{ "Test 1", "Test2" }, &iTest));
						Column1->AddChild(new CSliderF("Test", "Test Tooltip", 1, 100, &fTest));
					}
					Tab3->AddChild(Column1);
					CVerticalLayout* Column2 = new CVerticalLayout("Column 2");
					{
						Column2->AddChild(new CCheckbox("Test", "Test Tooltip", &bTest));
						Column2->AddChild(new CCombobox("Test", "Test Tooltip", std::vector<std::string>{ "Test 1", "Test2" }, &iTest));
						Column2->AddChild(new CSliderF("Test", "Test Tooltip", 1, 100, &fTest));
					}
					Tab3->AddChild(Column2);
				}
				Tab1->AddChild(Tab3);
				CSubTab* Tab4 = new CSubTab("Tab 4");
				{
					CVerticalLayout* Column3 = new CVerticalLayout("Column 3");
					{
						Column3->AddChild(new CCheckbox("Test", "Test Tooltip", &bTest));
						Column3->AddChild(new CCombobox("Test", "Test Tooltip", std::vector<std::string>{ "Test 1", "Test2" }, &iTest));
						Column3->AddChild(new CSliderF("Test", "Test Tooltip", 1, 100, &fTest));
					}
					Tab4->AddChild(Column3);
					CSplitVerticalLayout* SplitColumn1 = new CSplitVerticalLayout("Split Column 1", "Split Column 1 Bottom", 1);
					{
						SplitColumn1->AddChild(new CCheckbox("Test", "Test Tooltip", &bTest));
						SplitColumn1->AddChild(new CCombobox("Test", "Test Tooltip", std::vector<std::string>{ "Test 1", "Test2" }, &iTest));
						SplitColumn1->AddChild(new CSliderF("Test", "Test Tooltip", 1, 100, &fTest));
					}
					Tab4->AddChild(SplitColumn1);
				}
				Tab1->AddChild(Tab4);
			}
			TestWindow->AddChild(Tab1);
			CTab* Tab2 = new CTab("Tab 2");
			{
				CSubTab* Tab5 = new CSubTab("Tab 5");
				{
					CSplitVerticalLayout* SplitColumn2 = new CSplitVerticalLayout("Split Column 2", "Split Column 2 Bottom", 3);
					{
						SplitColumn2->AddChild(new CCheckbox("Test", "Test Tooltip", &bTest));
						SplitColumn2->AddChild(new CCombobox("Test", "Test Tooltip", std::vector<std::string>{ "Test 1", "Test2" }, &iTest));
						SplitColumn2->AddChild(new CSliderF("Test", "Test Tooltip", 1, 100, &fTest));
					}
					Tab5->AddChild(SplitColumn2);
					CVerticalLayout* Column4 = new CVerticalLayout("Column 4");
					{
						Column4->AddChild(new CCheckbox("Test", "Test Tooltip", &bTest));
						Column4->AddChild(new CCombobox("Test", "Test Tooltip", std::vector<std::string>{ "Test 1", "Test2" }, &iTest));
						Column4->AddChild(new CSliderF("Test", "Test Tooltip", 1, 100, &fTest));
					}
					Tab5->AddChild(Column4);
				}
				Tab2->AddChild(Tab5);
				CSubTab* Tab6 = new CSubTab("Tab 6");
				{
					CSplitVerticalLayout* SplitColumn3 = new CSplitVerticalLayout("Split Column 3", "Split Column 3 Bottom", 1);
					{
						SplitColumn3->AddChild(new CCheckbox("Test", "Test Tooltip", &bTest));
						SplitColumn3->AddChild(new CCombobox("Test", "Test Tooltip", std::vector<std::string>{ "Test 1", "Test2" }, &iTest));
						SplitColumn3->AddChild(new CSliderF("Test", "Test Tooltip", 1, 100, &fTest));
					}
					Tab6->AddChild(SplitColumn3);
					CSplitVerticalLayout* SplitColumn4 = new CSplitVerticalLayout("Split Column 4", "Split Column 4 Bottom", 2);
					{
						SplitColumn4->AddChild(new CCheckbox("Test", "Test Tooltip", &bTest));
						SplitColumn4->AddChild(new CCombobox("Test", "Test Tooltip", std::vector<std::string>{ "Test 1", "Test2" }, &iTest));
						SplitColumn4->AddChild(new CSliderF("Test", "Test Tooltip", 1, 100, &fTest));
					}
					Tab6->AddChild(SplitColumn4);
				}
				Tab2->AddChild(Tab6);
			}
			TestWindow->AddChild(Tab2);
		}
		BaseObjects.push_back(TestWindow);
		*/
	...
	void DrawCursor()
	{
		int x = CursorPos.x;
		int y = CursorPos.y;
		DrawIntRect(IntRect{ x, y, 1, 1 }, Color(0, 0, 0, MenuAlpha));
		DrawIntRect(IntRect{ x, y + 1, 1, 1 }, Color(0, 0, 0, MenuAlpha));
		DrawIntRect(IntRect{ x, y + 2, 1, 1 }, Color(0, 0, 0, MenuAlpha));
		DrawIntRect(IntRect{ x, y + 3, 1, 1 }, Color(0, 0, 0, MenuAlpha));
		DrawIntRect(IntRect{ x, y + 4, 1, 1 }, Color(0, 0, 0, MenuAlpha));
		DrawIntRect(IntRect{ x, y + 5, 1, 1 }, Color(0, 0, 0, MenuAlpha));
		DrawIntRect(IntRect{ x, y + 6, 1, 1 }, Color(0, 0, 0, MenuAlpha));
		DrawIntRect(IntRect{ x, y + 7, 1, 1 }, Color(0, 0, 0, MenuAlpha));
		DrawIntRect(IntRect{ x, y + 8, 1, 1 }, Color(0, 0, 0, MenuAlpha));
		DrawIntRect(IntRect{ x, y + 9, 1, 1 }, Color(0, 0, 0, MenuAlpha));

		DrawIntRect(IntRect{ x, y, 1, 1 }, Color(0, 0, 0, MenuAlpha));
		DrawIntRect(IntRect{ x + 1, y + 1, 1, 1 }, Color(0, 0, 0, MenuAlpha));
		DrawIntRect(IntRect{ x + 2, y + 2, 1, 1 }, Color(0, 0, 0, MenuAlpha));
		DrawIntRect(IntRect{ x + 3, y + 3, 1, 1 }, Color(0, 0, 0, MenuAlpha));
		DrawIntRect(IntRect{ x + 4, y + 4, 1, 1 }, Color(0, 0, 0, MenuAlpha));
		DrawIntRect(IntRect{ x + 5, y + 5, 1, 1 }, Color(0, 0, 0, MenuAlpha));
		DrawIntRect(IntRect{ x + 6, y + 6, 1, 1 }, Color(0, 0, 0, MenuAlpha));

		DrawIntRect(IntRect{ x + 1, y + 2, 1, 1 }, Color(0, 128, 255, MenuAlpha));
		DrawIntRect(IntRect{ x + 1, y + 3, 1, 1 }, Color(0, 128, 255, MenuAlpha));
		DrawIntRect(IntRect{ x + 1, y + 4, 1, 1 }, Color(0, 128, 255, MenuAlpha));
		DrawIntRect(IntRect{ x + 1, y + 5, 1, 1 }, Color(0, 128, 255, MenuAlpha));
		DrawIntRect(IntRect{ x + 1, y + 6, 1, 1 }, Color(0, 128, 255, MenuAlpha));
		DrawIntRect(IntRect{ x + 1, y + 7, 1, 1 }, Color(0, 128, 255, MenuAlpha));

		DrawIntRect(IntRect{ x + 2, y + 3, 1, 1 }, Color(0, 128, 255, MenuAlpha));
		DrawIntRect(IntRect{ x + 2, y + 4, 1, 1 }, Color(0, 128, 255, MenuAlpha));
		DrawIntRect(IntRect{ x + 2, y + 5, 1, 1 }, Color(0, 128, 255, MenuAlpha));
		DrawIntRect(IntRect{ x + 2, y + 6, 1, 1 }, Color(0, 128, 255, MenuAlpha));
		DrawIntRect(IntRect{ x + 2, y + 7, 1, 1 }, Color(0, 128, 255, MenuAlpha));

		DrawIntRect(IntRect{ x + 3, y + 4, 1, 1 }, Color(0, 128, 255, MenuAlpha));
		DrawIntRect(IntRect{ x + 3, y + 5, 1, 1 }, Color(0, 128, 255, MenuAlpha));
		DrawIntRect(IntRect{ x + 3, y + 6, 1, 1 }, Color(0, 128, 255, MenuAlpha));

		DrawIntRect(IntRect{ x + 4, y + 5, 1, 1 }, Color(0, 128, 255, MenuAlpha));
		DrawIntRect(IntRect{ x + 4, y + 6, 1, 1 }, Color(0, 128, 255, MenuAlpha));

		DrawIntRect(IntRect{ x, y + 8, 1, 1 }, Color(0, 0, 0, MenuAlpha));
		DrawIntRect(IntRect{ x + 1, y + 8, 1, 1 }, Color(0, 0, 0, MenuAlpha));
		DrawIntRect(IntRect{ x + 2, y + 8, 1, 1 }, Color(0, 0, 0, MenuAlpha));
		DrawIntRect(IntRect{ x + 3, y + 7, 1, 1 }, Color(0, 0, 0, MenuAlpha));
		DrawIntRect(IntRect{ x + 4, y + 7, 1, 1 }, Color(0, 0, 0, MenuAlpha));
		DrawIntRect(IntRect{ x + 5, y + 6, 1, 1 }, Color(0, 0, 0, MenuAlpha));
